...
t = odesoln.x';
x = odesoln.y';
tp = linspace(tspan(1), tspan(2), 300)';
xp = deval(odesoln, tp);
xp = xp';
...
for i = 1:length(t)-1
    ...
    y(:,i+1) = y(:,i) + h(i)*(b(1)*k1 + b(2)*k2 + b(3)*k3 + b(4)*k4 + b(5)*k5 + b(6)*k6 + b(7)*k7);
...
% for i = 1:length(t)-1
%     J = find(t(i+1)>tp&tp>t(i));
%     tinterp = tp(J);
%     s = (tinterp - t(i))/h;
%     
%     yinterp = y(:,ones(size(tinterp))) + f*(h(i)*BI)*cumprod([s;s;s;s]);
%     
% end
...
xsp = interp1(t, x, tp, 'spline');
ysp = interp1(t, y, tp, 'spline');
...
plot3(xsp(:,1),xsp(:,2),xsp(:,3), 'color', 'black')
plot3(xsp(:,1)+ysp(:,1),xsp(:,2)+ysp(:,2),xsp(:,3)+ysp(:,3))
