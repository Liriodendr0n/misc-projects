function [etaQ, wQ, etaC, wC] = spanLattice(N, Ltip, Rtip)
%spanLattice Generates spanwise discretization, Ltip Rtip true/false
%   Detailed explanation goes here

xw = jacobi(N+1, -0.5*Rtip, -0.5*Ltip, nonzeros([-1*~Ltip, 1*~Rtip]));
etaQ = xw(:,1);
wQ = xw(:,2);
...
for ci = 1:N-1
    xC(ci) = fzero(f, [xQ(ci)+1e-6, xQ(ci+1)-1e-6]);
end
