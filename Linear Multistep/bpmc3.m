function [t, y, f] = bpmc3(f, tspan, h, stencil, y0)
...
for ti = 1:length(ts)
    ...
    ytp1c1 = yt(:,ti) + h*sum(g(2:end).*fhist(:,1:end-1), 2) + h*g(1)*f(ts(ti)+h, ytp1p(:,1));
    ytp1c2 = yt(:,ti) + h*sum(g(2:end).*fhist(:,1:end-1), 2) + h*g(1)*f(ts(ti)+h, ytp1c1(:,1));
    ytp1 = yt(:,ti) + h*sum(g(2:end).*fhist(:,1:end-1), 2) + h*g(1)*f(ts(ti)+h, ytp1c2(:,1));
